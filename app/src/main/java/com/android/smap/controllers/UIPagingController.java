 ...
 *
 * @param <T> Clean model type of each page item.
 * @param <V> Dirty raw model from feed of underlying concrete controller
...
public abstract class UIPagingController<T, V> extends UIRequestController<V> {
     ...
     * @return
     */
    protected abstract HttpUrlRequest<?> getPageRequest(int pageSize, int page);
